...
class AccruedRevenueXLSX(models.AbstractModel):
    ...
    def _generate_month_sheets(self, workbook, formats, filtered_lines, all_lines, accrual_month, accrued_account_id):
            ...
            sheet.write_formula(total_row, 5, f'=SUM(F{data_start_row + 1}:F{total_row})', currency_bold_format)
            sheet.write_formula(total_row, 6, f'=SUM(G{data_start_row + 1}:G{total_row})', currency_negative_bold_format)
            sheet.write_formula(total_row, 7, f'=SUM(H{data_start_row + 1}:H{total_row})', currency_negative_bold_format)
            sheet.write_formula(total_row, 8, f'=SUM(I{data_start_row + 1}:I{total_row})', currency_negative_bold_format)
            sheet.write_formula(total_row, 9, f'=SUM(J{data_start_row + 1}:J{total_row})', currency_negative_bold_format)
            sheet.write_formula(total_row, 10, f'=SUM(K{data_start_row + 1}:K{total_row})', currency_negative_bold_format)
